...
if __name__ == "__main__":
	...
	while len(host_list) > 0:
		...
		dst = random.randint(16, nhost-1)
		
		if inter_flow_cnt == inter_flow_cnt_limit and src <= 15:
			src = random.randint(16, nhost-1)
		...
		# 不能再生成DC间的流量了
		while ((inter_flow_cnt == inter_flow_cnt_limit) and (src <= 15 and dst >= 16 or src >= 16 and dst <= 15)):
			dst = random.randint(16, nhost-1)
			while (dst == src):
				dst = random.randint(16, nhost-1)
		# 不能再生成DC内部的流量了
		while ((intra_flow_cnt == intra_flow_cnt_limit) and (src <= 15 and dst <= 15 or src >= 16 and dst >= 16)):
			dst = random.randint(16, nhost-1)
			while (dst == src):
				dst = random.randint(16, nhost-1)
		...
		if (t + inter_t > time + base_t):
			...
			if (src <= 15 and dst >= 16 or src >= 16 and dst <= 15):
				inter_flow_cnt += 1
			else:
				intra_flow_cnt += 1
