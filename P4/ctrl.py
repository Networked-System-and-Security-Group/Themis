...
class TimerPktgenTest(BfRuntimeTest):
    ...
    def runTest(self):
        logger.info("=============== Themis Setup ===============")
        bfrt_info = self.interface.bfrt_info_get("themis")

        # 获取的是系统table，并不是用户自己定义的，通过向这些table添加表项来控制tofino
        ...
        # 根据ip转发
        self.ip_table = bfrt_info.table_get("SwitchIngress.ip_table")
        self.ip_table = bfrt_info.table_get("SwitchIngress.cnp_gen_table")
        ...
        try:
            ...
            if g_is_tofino:
                data = pktgen_app_cfg_table.make_data([gc.DataTuple('timer_nanosec', 100),
                                                      ...
                                                      '$PKTGEN_TRIGGER_TIMER_PERIODIC') #PKTGEN_TRIGGER_TIMER_ONE_SHOT
            if g_is_tofino2:
                data = pktgen_app_cfg_table.make_data([gc.DataTuple('timer_nanosec', 100),
                                                       gc.DataTuple('app_enable', bool_val=False),
                                                       gc.DataTuple('pkt_len', (pktlen - 6)),
