...
typedef struct count_cnp_list_tuple_s{
	uint32_t opcode;
} count_cnp_list_tuple_t;

typedef struct count_cnp_info_s {
    // Key field ids
    bf_rt_id_t kid_opcode;
    // Action Ids
    bf_rt_id_t aid_inc_cnp_counter;
    // Key and Data objects
    bf_rt_table_key_hdl *key;
    bf_rt_table_data_hdl *data;
} count_cnp_info_t;

typedef struct count_cnp_entry_s {
    // Key value
    uint32_t opcode;
    // Action
    char action[20];
...
typedef struct set_cnp_dstQPN_info_s {
    ...
    // Data field Ids for forward
    bf_rt_id_t did_clientQPN;
    // Key and Data objects
    bf_rt_table_key_hdl *key;
    bf_rt_table_data_hdl *data;
} set_cnp_dstQPN_info_t;
